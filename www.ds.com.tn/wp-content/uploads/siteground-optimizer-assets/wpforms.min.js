...
$field.removeClass('wpforms-has-error');},submitHandler:function(form){var $form=$(form),$submit=$form.find('.wpforms-submit'),altText=$submit.data('alt-text'),recaptchaID=$submit.get(0).recaptchaID;if(!app.empty(recaptchaID)||recaptchaID===0){grecaptcha.execute(recaptchaID);return false;}
if(altText){$submit.text(altText);}
$submit.prop('disabled',true);$('.wpforms-input-temp-name').removeAttr('name');app.formSubmit($form);},onkeyup:function(element,event){var excludedKeys=[16,17,18,20,35,36,37,38,39,40,45,144,225];if($(element).hasClass('wpforms-novalidate-onkeyup')){return;}
...
if(13===event.which){$('#'+$this.attr('for')).click();}});$(document).on('change','.wpforms-field-checkbox input, .wpforms-field-radio input, .wpforms-field-payment-multiple input, .wpforms-field-payment-checkbox input, .wpforms-field-gdpr-checkbox input',function(event){var $this=$(this),$field=$this.closest('.wpforms-field');if($field.hasClass('wpforms-conditional-hide')){event.preventDefault();return false;}
...
$submitBtn.prop('disabled',true);}else{$errorCnt.find('.wpforms-error-container-post_max_size').remove();$submitBtn.prop('disabled',false);}});$(document).on('keydown','.wpforms-form input',function(e){if(e.keyCode!==13){return;}
...
if('next'===action){if(typeof $.fn.validate!=='undefined'){$page.find(':input').each(function(index,el){if(!$(el).valid()){valid=false;}});var $topError=$page.find('.wpforms-error').first();if($topError.length){app.animateScrollTop($topError.offset().top-75,750,$topError.focus);}}
...
if(!app.empty(recaptchaID)||recaptchaID===0){grecaptcha.reset(recaptchaID);}},consoleLogAjaxError:function(error){if(error){console.error('WPForms AJAX submit error:\n%s',error);}else{console.error('WPForms AJAX submit error');}},displayFormAjaxErrors:function($form,errors){if(app.empty(errors)||(app.empty(errors.general)&&app.empty(errors.field))){app.consoleLogAjaxError();return;}
...
var $container=$form.closest('.wpforms-container'),$spinner=$form.find('.wpforms-submit-spinner'),$confirmationScroll,formData,args;$container.css('opacity',0.6);$spinner.show();app.clearFormAjaxGeneralErrors($form);formData=new FormData($form.get(0));formData.append('action','wpforms_submit');args={type:'post',dataType:'json',url:wpforms_settings.ajaxurl,data:formData,cache:false,contentType:false,processData:false,};args.success=function(json){if(!json){app.consoleLogAjaxError();return;}
if(!json.success){app.resetFormRecaptcha($form);var errors=json.data&&'errors'in json.data?json.data.errors:null;app.displayFormAjaxErrors($form,errors);$form.trigger('wpformsAjaxSubmitFailed',json);return;}
...
if(json.data.confirmation){$container.html(json.data.confirmation);$confirmationScroll=$container.find('div.wpforms-confirmation-scroll');if($confirmationScroll.length){app.animateScrollTop($confirmationScroll.offset().top-100);}}};args.error=function(jqHXR,textStatus,error){app.consoleLogAjaxError(error);$form.trigger('wpformsAjaxSubmitError',[jqHXR,textStatus,error]);};args.complete=function(jqHXR,textStatus){var $submit=$form.find('.wpforms-submit'),submitText=$submit.data('submit-text');if(submitText){$submit.text(submitText).prop('disabled',false);}
